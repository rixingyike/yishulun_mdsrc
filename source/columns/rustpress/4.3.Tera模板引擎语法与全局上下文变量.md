...
> Tera 模板引擎提供了强大的逻辑控制与过滤器支持，赋予了前端极大的布局自由度；但如果在 HTML 模板中强行编写深度嵌套的多层计算或复杂字符串清洗，会破坏模板的可读性与渲染性能。  
> **边界原则**：模板层仅负责「如何将已有数据渲染为 HTML」；所有复杂的数据清洗、排序算法与倒分页计算均在 Rust 内核预先算好并直接注入上下文。分界在于「是视图排版还是数据计算」。
