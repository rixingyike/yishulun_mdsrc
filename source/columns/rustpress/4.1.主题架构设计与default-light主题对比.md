...
> **辩证权衡：原子化样式 vs 模板组件化**  
> Tailwind CSS 原子类支持在 HTML 中就地声明样式，免去了在样式表间反复横跳与起名困难；但在复杂布局中过度堆砌原子类会导致 HTML 标签极其臃肿、重复难以维护。  
> **边界原则**：页面局部的微调与排版直接使用原子类；跨页面高频复用的 UI 结构封装进 `components/*.html` 独立组件。分界在于「样式属于局部排版还是全站公共组件」。
